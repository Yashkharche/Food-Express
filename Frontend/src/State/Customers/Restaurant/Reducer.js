...
const restaurantReducer = (state = initialState, action) => {
  ...
  return {
    ...state,
    loading: false,
    events: state.events ? state.events.filter((item) => item.id !== action.payload) : [],
    restaurantsEvents: state.restaurantsEvents
      ? state.restaurantsEvents.filter((item) => item.id !== action.payload)
      : [],
  };
